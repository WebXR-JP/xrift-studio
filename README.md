...
| AIと一緒にSceneを編集する | 検証中 | 検証中 | CodexまたはClaude Codeをアプリから登録し、開いているSceneの読取・設定変更・Asset配置を限定MCP toolで行う。 |
| 外部ツールで作業を続ける | 対応 | 対応 | プロジェクトを VS Code またはターミナルで開く。Visual projectから通常のXRiftコードプロジェクトへ移植するCLIは計画中。 |
...
## Visual projectを通常のXRift開発へ渡す構想

Visual Editorで作ったproject JSONとAssets一式を、既存のXRiftコードプロジェクトへ安全に引き渡す`npx`コマンドを計画しています。

```bash
npx @xrift/studio-cli migrate ./my-visual-project/xrift-studio.project.json --to ./my-xrift-world
```

このコマンドは**まだ実装されていません**。初期版はread-onlyの検査とdry-runから始め、生成コードとAssetをmanaged領域へ書き出します。手書きファイルを既定で上書きせず、衝突検知、再実行時の差分、migration記録を備える方針です。詳しい仕様と実装順序は[Visual Project Migration CLI](./docs/VISUAL_PROJECT_MIGRATION_CLI.md)を参照してください。
...
ビジュアルエディターのAI connection panelは、インストール済みのCodexまたはClaude Codeを検出し、XRift Studio MCP serverを登録できます。登録後にAI clientを再起動すると、開いているSceneの読取、Entity更新、Asset配置など、許可されたEditor toolを利用できます。
