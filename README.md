...
| AIと一緒にSceneを編集する | 検証中 | 検証中 | Codex、Claude Code、Claude Desktop / Coworkをアプリから登録し、開いているSceneの読取・設定変更・Asset配置を限定MCP toolで行う。 |
...
現在はrepository内の開発版で、`--dry-run`、空folderへの書出し、未改変exportの`--update`、衝突検知、provenanceを利用できます。生成する`public/xrift/runtime.json`は`xrift-studio-runtime/three`または`xrift-studio-runtime/react-three-fiber`から読み込み、Classicの`World.tsx`／`Item.tsx`は薄いadapterだけを持ちます。npm公開とRuntime Component対応状況は[Visual Project Classic Export CLI](./docs/VISUAL_PROJECT_MIGRATION_CLI.md)を参照してください。
...
ビジュアルエディターのAI connection panelは、インストール済みのCodex、Claude Code、Claude Desktop / Coworkを検出し、XRift Studio MCP serverを登録できます。登録後にAI clientを再起動すると、開いているSceneの読取、Entity更新、Asset配置など、許可されたEditor toolを利用できます。Claude Desktop / Coworkはローカルsessionで利用でき、remote CoworkではローカルMCP serverを起動できません。
