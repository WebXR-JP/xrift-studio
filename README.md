...
登録後にAIクライアントを再起動するか、MCPを再読み込みすると、開いているSceneの読み取り、Entityの更新、Assetの配置など、許可された操作を利用できます。プロジェクトの作成と開く操作、ログインの開始、公開もMCPから行えます。公開は`publish_project`が公開情報・サムネイル・ログイン・公開チェックを確認してから、画面の公開と同じ手順で実行します。同梱の`xrift-studio-mcp-sidecar`を使うと、同じ操作をCLIからも呼べます。
...
MCPサーバーはTauri側の中継処理を介して、起動中のStudioへ接続します。リクエストのサイズ、利用できるツール名、応答の待ち時間を制限します。プロジェクトが開いていないときは、作成・開く・アカウントの操作だけを受け付け、編集の操作には開くよう案内するエラーを返します。配布時は`pnpm mcp:sidecar:prepare:release`でserver binaryを準備し、Tauri sidecarとして同梱します。
