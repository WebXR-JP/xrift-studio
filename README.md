...
| 新しいプロジェクトを作る | 対応 | 対応 | 種別と名前を選んでテンプレートから作成し、作成後のプロジェクトをそのまま開く。 |
...
| XRift に公開する | 対応 | 対応 | ログイン後、タイトル・説明・サムネイルを公開前に確認する。初期状態なら編集へ案内し、準備が整ってから種別に応じてアップロードする。 |
...
4. プロジェクトライブラリで **新規プロジェクト** を選び、**ワールド** または **アイテム** を作成します。
