...
アプリは起動後に GitHub Releases の最新版を確認します。新しい署名済みバージョンがある場合は、現在版、最新版、更新内容を確認して **更新して再起動** を選べます。**後で** を選んだ場合も、プロジェクトライブラリ上部の更新表示または About の **更新を確認** から再開できます。

ダウンロードとインストール中は進捗を表示し、完了後に XRift Studio を自動で再起動します。確認やインストールに失敗しても現在のアプリは維持され、同じ画面から再試行できます。

この更新機能をまだ含まない v0.4.0 以前から、最初の更新対応版へ移る時だけは GitHub Releases から手動でインストールしてください。その後の更新はアプリ内で行えます。
