...
fn tool_definitions() -> Value {
    json!([
        ...
        },
        {
            ...
            "description": "Resume persistent world authoring: return the blueprint, criteria, capture IDs, reviews, nextActions and worldComponents (usage/placement guidance and current document component instances). Revisit the screenShareDecision before decorating a social, collaborative or presentation world. Scene content changes invalidate previous captures even across app restarts. Follow nextActions; the LLM judges image quality and functional evidence, Studio checks capture freshness. Component presence or an Edit placeholder does not prove sharing or synchronization works.",
        ...
        },
        {
            ...
            "description": "Read the Script authoring workflow; xrift:script lifecycle, Asset, targeted Material, and Particle runtime APIs; persistent authoring tools; and the explicit sandboxed:false, trustGate:false execution boundary.",
            "inputSchema": { "type": "object", "properties": {}, "additionalProperties": false }
        },
        {
        ...
        {
            "name": "import_font_asset",
            "description": "Validate and import one trusted local font file into managed project storage while Edit is active. TrueType (.ttf), OpenType (.otf) and WOFF 1.0 (.woff) are accepted; WOFF2 is refused because the text renderer cannot parse it. sourcePath must be an absolute path to a regular non-symlink file no larger than 64 MB. Extension and file signature must agree. The result returns the Font Asset and project-relative destination, never file bytes, a data URL, or the external source path. Reference the imported Asset from a Text component with update_component patch.fontAssetId.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "projectId": { "type": "string" },
        ...
        {
            "name": "set_play_mode",
            "description": "Start or stop the visual editor Play session. Starting Play compiles saved Scripts and executes them without a separate approval dialog, matching the Studio Play button. Compilation failure leaves the editor in Edit mode. Read get_editor_context.scriptRuntime afterward to check runtime failures. Script execution is not sandboxed.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "projectId": { "type": "string" },
                    ...
                    "unapprovedPolicy": {
                        "type": "string",
                        "enum": ["skip"],
                        "description": "Legacy compatibility field. Script approval has been removed; this value no longer changes execution."
                    }
                },
        ...
        },
        {
            ...
            "description": "Use the user's configured TypeSafe Jev API to compose a world from XRift Studio's real catalogs. It chooses Terrain, Terrain Surface, grass preset, Skybox mode or Sky Shader preset, mood, optional post-processing finish, vegetation wind, density, scale, composition, detail level, humanized placement strength, multiple role-specific Scene Recipe archetypes, repetition counts, official XRift facilities, placement anchors, and optional Primitive helpers. One archetype can expand to repeated placements such as trees, rocks, benches or lights. There is no fixed global placement cap; each recipe family keeps a bounded per-archetype repeat limit so large worlds can exceed 50 placements without unbounded single-asset spam. Roles cover signature, landscape, furniture/structure, lighting, weather/effects, interactions, and material showcases. Jev only selects from candidates XRift Studio supplies; it cannot invent tool names, recipe IDs, coordinates, code, or delete anything. Existing Entity positions are considered when resolving placements. This tool does not mutate the Scene. It returns decisionTrace, actionPlan, terrainSurfacePlan, grassPlan, skyboxPlan, facilityPlan, primitivePlan, and grounding instructions. After Terrain creation or when an existing Terrain is present, apply the selected surface/grass, resolve Skybox, sample the ground before placements, then capture_scene_view. If Jev is not configured, ask the person to enter their TypeSafe API key in Studio's AI connection panel.",
            "inputSchema": {
                "type": "object",
                "properties": {
        ...
        },
        {
            ...
            "description": "Download a Poly Haven or ambientCG asset into the open project and create XRift Studio assets: an HDRI becomes an environment Texture (applySkybox makes it the sky at once), a texture bundle becomes a PBR Material ready for set_material, and a Poly Haven Model is validated and saved as a self-contained glTF Model Asset to place with place_asset. Choose resolution from the download budget set for the world; larger textures multiply its size. Read polycount from search_external_assets before installing a model. ambientCG Models remain catalog-only until a compatible glTF download is available.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "projectId": { "type": "string" },
        ...
        },
        {
            ...
            "description": "Update persisted Skybox, Sky Shader, Fog, ambient light, camera, post effects, and editor viewport settings through XRift Studio history and autosave. postprocessing.order sets which post effect layer is applied first, which changes the look of the same settings; postprocessing.ao/bloom/grading each switch one layer on or off. skybox.materialAssetId assigns a Custom Shader Material as the procedural sky; it draws the background instead of the image and the gradient. This is supported during Edit and Play; Play reflects the shared Scene settings immediately. Whether post effects belong is the blueprint's call: a request for a high-end look may want bloom or grading, a light world may not. They change every viewer's frame on WebGL and cost frame time on weak devices, so judge them by a capture_scene_view comparison rather than by habit.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "projectId": { "type": "string" },
        ...
        },
        {
            ...
            "description": "List the central Editor Component registry with stable definition IDs, categories, project-kind availability, multiplicity, and required Asset kinds. World projects also return worldComponents: experience selection, official field definitions, builtin recipe IDs, editable prefab fields, placement/verification guidance and current component instances. Consider ScreenShareDisplay first for sharing work or presentations; Mirror for avatar viewing; TagBoard for event tag selection. Call this instead of guessing add_component definitionId values. Use place_builtin_prefab for a ready official component, or create_empty_entity/add_component/update_component for custom dimensions and properties; verify from actual usage viewpoints.",
            "inputSchema": { "type": "object", "properties": {}, "additionalProperties": false }
        },
        {
        ...
        },
        {
            ...
            "description": "Create a builtin primitive shape (box, sphere, cylinder, cone, or plane) as a new scene entity. This is the blockout tool: floors, walls, platforms, steps, counters, pillars and abstract structures start as primitives with a real Material (materialAssetId here, or set_material afterwards) and are sized through update_transform. A box floor carrying a real Material (a PBR bundle from install_external_asset, a catalog preset, or one authored here) is one good ground for rooms, streets, stages, plazas and floating worlds; a Terrain is only for landscapes. Do not leave the default grey Material on a primitive that ships.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "projectId": { "type": "string" },
        ...
        {
            "name": "create_terrain",
            "description": "Create a static height-sampled Terrain with a fixed Trimesh Collider. It starts flat; use sculpt_terrain for deterministic Raise, Lower, Set Height, Smooth, Stamp, and Paint Holes operations. For a flat floor use create_primitive instead: a Terrain costs a height field and a Trimesh Collider and only pays off when the ground has relief.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "projectId": { "type": "string" },
        ...
        },
        {
            ...
            "description": "List the Terrain grass catalog: every grass type with its default blade size and colours, every one-step preset with the layers it expands into, and the density, slope, height-band and per-layer instance limits the grass tools enforce. Call this before adding or tuning a grass layer instead of guessing typeId or presetId values. Read this to pick a preset; a tuning pass starts from a capture_scene_view frame that shows a defect, not from this list.",
            "inputSchema": { "type": "object", "properties": {}, "additionalProperties": false }
        },
        {
            "name": "apply_terrain_grass_preset",
            "description": "Replace a Terrain's grass layers with one preset's stack, matching the Inspector's one-step vegetation button. This discards existing layers, including any painted coverage; use add_terrain_grass_layer to add to a stack instead. How much grass matters is the blueprint's call. The trap is retuning it blind: change a layer after a capture_scene_view frame shows a concrete defect (colour that fights the sky, blades floating over a path, bald patches), not as a substitute for adding a landmark, props or lighting.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "projectId": { "type": "string" },
        ...
        },
        {
            ...
            "description": "Place a builtin official XRift component into the scene. Read list_component_definitions worldComponents for purpose and placement guidance: ScreenShareDisplay for live screen sharing (not VideoScreen), Mirror for avatar viewing, TagBoard for event tag selection. Reserve audience, operator and circulation areas before placement. The response includes componentId and placementGuidance. After placement, inspect get_entity_components, orient and size with update_transform, and capture_scene_view from audience/operator positions. Prefab properties are protected except listed editable fields; for custom component dimensions use create_empty_entity/add_component/update_component instead. Test sharing/tag selection in a supported runtime; a placed placeholder is not functional verification.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "projectId": { "type": "string" },
