...
fn tool_definitions() -> Value {
    json!([
        ...
        },
        {
            ...
            "description": "Update persisted Skybox, Sky Shader, Fog, ambient light, camera, post effects, and editor viewport settings through XRift Studio history and autosave. postprocessing.order sets which post effect layer is applied first, which changes the look of the same settings; postprocessing.ao/bloom/grading each switch one layer on or off. skybox.materialAssetId assigns a Custom Shader Material as the procedural sky; it draws the background instead of the image and the gradient. This is supported during Edit and Play; Play reflects the shared Scene settings immediately. Post effects are off by default and stay off unless the brief asks for the look: they change every viewer's frame on WebGL, cost frame time on weak devices, and most worlds read better without them, so enable one only after a capture_scene_view comparison shows the gain.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "projectId": { "type": "string" },
