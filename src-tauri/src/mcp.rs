...
fn tool_definitions() -> Value {
    json!([
        ...
        },
        {
            ...
            "description": "Use the user's configured TypeSafe Jev API to compose a world from XRift Studio's real catalogs. It chooses Terrain, Terrain Surface, grass preset, mood, post-processing finish, vegetation wind, density, scale, composition, detail budget, multiple role-specific Scene Recipe archetypes, repetition counts, official XRift facilities, placement anchors, and optional Primitive helpers. One archetype can expand to repeated placements such as trees, rocks, benches or lights, with a hard total cap of 50 generated placements. Roles cover signature, landscape, furniture/structure, lighting, weather/effects, interactions, and material showcases. Jev only selects from candidates XRift Studio supplies; it cannot invent tool names, recipe IDs, coordinates, code, or delete anything. Existing Entity positions are considered when resolving placements. This tool does not mutate the Scene. It returns decisionTrace, actionPlan, terrainSurfacePlan, grassPlan, facilityPlan, primitivePlan, and grounding instructions. After Terrain creation or when an existing Terrain is present, apply the selected surface/grass, sample the ground before placements, then capture_scene_view. If Jev is not configured, ask the person to enter their TypeSafe API key in Studio's AI connection panel.",
            "inputSchema": {
                "type": "object",
                "properties": {
                    "prompt": { "type": "string", "description": "Short description of the world to build." },
                    "maxElements": { "type": "integer", "minimum": 1, "maximum": 50, "description": "Hard cap for generated placements; defaults to 50. Jev may choose a smaller detail budget from the prompt." }
                },
                "required": ["prompt"],
                "additionalProperties": false
            }
