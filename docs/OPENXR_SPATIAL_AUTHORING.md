...
ビルド時はOpenXR Loaderの同梱にCMakeとC++ツールチェーンが必要。利用者がLoader DLLを配置する手順は不要。ネイティブ取得・取消はSessionをまたぐ非同期操作のため、現在の同期MCP document surfaceには公開していない。UIとTauri IPCに限定し、MCPの簡易形状配置を直接取得と呼ばない。

## 操作と結果

Scene Viewの入口は「部屋を取り込む」。ダイアログは画面内に収め、キーボードのフォーカスを内側に保つ。閉じる操作とEscapeで元のボタンへ戻る。閉じても処理は継続し、ツールバーに進行状態を残す。

- 「Meta Quest」「PICO」「その他」で準備と対応状況を切り替える。機種の選択は案内用で、端末の検出結果として扱わない。PICOは未対応を示し、取得操作を出さない。
- 初回表示と「接続を確認し直す」はRuntimeの必須拡張だけを確認する。成功表示を「接続済み」や実機取得の保証に置き換えない。拡張名やエラーの詳細は折りたたみ、利用者が取れる次の操作を先に示す。
- 取得・取消待ち・素材保存を区別し、取消を失敗扱いにしない。保存中の取消は提供しない。Scene変更時は既存の編集を保持する。
- 完了後は配置・除外件数と警告を残す。「取り込んだ部屋を表示」で配置Entityを選択して画面に収める。再取得は追加操作として示し、既存の部屋が残り重複することを実行前に伝える。
...
- UIは端末別の案内、必須機能の不足、Play中、取得・取消・保存、閉じて再表示、完了後の部屋表示、再取得時の重複案内を確認する。
- Windows実機では同梱Loaderを使い、読取、取消、切断、位置・向き、Scene変更時の拒否を確認する。

上記は確認手順であり、実行結果を表すものではない。実行した検証と結果はPRに記録する。コードレビューや型チェック・テストと、アプリの画面確認・実機取得の成功を区別する。実機取得は未検証で、新規スキャンやPICO対応を検証済みと扱わない。
