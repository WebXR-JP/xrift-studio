...
- iPadのパネル切り替え・視点だけ操作・複数選択・Playのタッチ入力は、端末側の表示または入力状態で、documentを変えないため専用toolを設けない。ブラウザの保存先選択、ZIPのダウンロードと共有メニューはユーザー操作とSafariの権限に依存するため、MCPから起動しない。プロジェクトのデータ編集とデスクトップのZIP書き出し・取り込みは既存toolを使う。iPadブラウザ内でのMCP接続は提供しない。
