...
- EditorのAI連携panelはCodex、Claude Code、Claude Desktop / Cowork、OpenCode、Cursorの検出結果、登録scope、XRift Studio MCP serverの状態を表示する。Codexは現在の`PATH`に加えて、公式installer、Codex app同梱CLI、npm、pnpm、WinGet、Homebrew、standalone installerの標準配置を確認する。起動時の環境変数が古い場合も再起動なしで検出する。native APIがないブラウザでは登録済みに見せない。「デスクトップ版で利用できます」と示す。Claude Desktop / Coworkはローカルsessionだけを対象にする。remote CoworkではローカルMCPを起動できないことを登録前に示す。
...
完了条件: 対応AIクライアントへXRift Studio MCPを一操作で登録する。認可したvisual projectの現在シーン、素材、selection、revisionを読み取る。空の背景 / Fog /環境光/Camera/Editor表示設定、素材配置、マテリアル編集、ノードグラフマテリアル pointer設定、Poly HavenとambientCGの検索・downloadを通常のEditor Command、元に戻す、Autosaveへ合流する。AIと手操作の競合を暗黙に上書きしない。登録後は接続状態、対象シーン、直近の編集と復帰手段がEditorに残る。
...
- シーンのtoolbarに「診断」を置く。編集と動作確認の両方で使え、動作確認の画面では動作確認中の実際のrendererを測る。短時間の診断動画は MCP の要求から取得する。
...
- MCP の診断動画要求はシーンのCanvasを最大15秒のWebMへ記録する。録画中であることと上限秒数を通知に出す。
...
- シーン右上の「録画」から開くパネルに、状態、フレーム (アスペクト比・短辺・fps と、その結果の画素数)、録画ビューの設定、録画用カメラ、保存先を 1 画面で置く。押す前に何がどの大きさで保存されるか読める。
