...
## F-47 文章・音声からワールドを作る（MI-124）

- 操作前: エディターの「ワールド作成」から専用パネルを開く。文章欄、入力例、音声入力、作成ボタンを表示する。Jev未設定なら同じパネルで自分のAPIキーを設定する。外部AIクライアントの検出やMCP登録は前提にしない。
- 入力中: Windowsでは文章欄にフォーカスしてOSの音声入力を呼び出す。ショートカット送信を録音開始と表示しない。macOS・LinuxではOS側の手順を案内する。認識結果を編集でき、作成は本人がボタンを押すまで始めない。
- 処理中: Jevへ一度問い合わせ、完成した構成を非表示の作業データとして組み立てる。処理段階を表示し、作成・キー変更・音声入力起動の同時実行を防ぐ。開始時のScene・プロジェクトを保持し、非同期処理後と反映直前に現在状態を照合する。
- 成功時: 完成したSceneと素材を一回の履歴に反映し、作成したまとまりを選択する。作成内容を残し、通常編集・Play・一回の元に戻すへつなげる。
- 失敗時: 完成前のSceneを反映せず、理由と再試行を表示する。入力文を保持する。別プロジェクトへの移動後に古い結果やエラーで新しいパネルを上書きしない。
- 戻り先: パネルを閉じても同じプロジェクトの入力文を残す。別プロジェクトへ切り替えた場合は下書きをリセットする。
