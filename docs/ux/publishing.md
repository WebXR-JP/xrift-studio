...
- 引き継ぎ: 書き出す拡張子は `.xriftstudio` とし、従来の `.zip` も取り込める。内部のZIP形式（xrift-studio.project.json、全シーン、Assets、Prefab、素材ファイルとpackage manifest）を維持する。Mac/Windows版の「ファイルから取り込む」へ案内する。公開記録を引き継がず、新しい未公開のプロジェクトとして開く。iPadの「公開」はこの受け渡しへ通し、トークン入力を表示しない。詳細は[プロジェクトの受け渡し形式](../PROJECT_PACKAGE.md)を参照する。
