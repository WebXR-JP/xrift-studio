...
- 狭い画面の表示: スマートフォンとタブレットの上部は作品名・保存状態と「作品」メニューへ集約する。素材追加・作品の切り替え・書き出し・シーン設定・ガイドをメニュー内へ移し、下部の常設Utility Railを外す。スマートフォンのAssetsは一覧まで一続きにスクロールでき、種類・フォルダー・並び順は必要に応じて開く。長い素材名を折り返し、マテリアル概要は設定値を圧迫しない大きさにする。
- ファイル操作: 「素材を追加」は編集中の作品への素材追加。「作品を開く」にブラウザ保存済み一覧と「.xriftstudioを開く」を統合し、作品を切り替える前に現在の保存を完了する。「.xriftstudioで保存」は素材を含めてファイルにまとめる。
...
- iPadの入口と入力: 紹介ページでは制作開始と保存済み作品を開く操作を優先する。新規作成は名前を入力して開き、一覧では更新日時と編集中の作品を識別できる。Assetsはフォルダーを保持したまま種類を絞り込める。タッチ用の数値欄は選択して打ち替えられ、負号・小数点など入力途中の値を保持する。画面サイズの監視は共有し、数値欄の端末判定はタッチ対応が変わったときだけ更新する。
- 保存: 同じdocument codecを使い、ブラウザのプロジェクトと素材をIndexedDBへ保存する。「ブラウザに保存済み」をファイルの保存完了と混同させない。直近のプロジェクトを復元し、「作品を開く」で保存済み一覧・新規作成・「.xriftstudioを開く」による取り込みを選べる。別プロジェクトを開く前に現在の保存を待つ。
