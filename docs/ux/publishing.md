...
- 操作前: PCのシーン・Hierarchy・Assets・Inspectorを共通化し、タブレットではパネルを切り替える。画面幅に合わせてシーンの横または下へ表示する。パネルの選択・視点だけ操作・複数選択はその場で切り替え、documentやPCのパネル幅を変更しない。iPhoneもエディターを直接表示し、読み込み後に短い操作モーダルを重ねる。閉じたらそのまま編集でき、同じページ内では繰り返し表示しない。スマートフォンは初期パネルを閉じ、シーン操作のボタン群を開閉できる。横向きではサイドパネルを使う。
