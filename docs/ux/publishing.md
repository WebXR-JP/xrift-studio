...
- 素材の追加: Hierarchy見出しの「追加 / 操作」に「外部から追加」と「ファイルから素材を追加」を置く。外部カタログは1023px以下でカテゴリ選択・一覧・詳細へ切り替え、素材を選んだら詳細、一覧へ戻ると選択したカードへ戻る。PCの3列表示を保つ。取り込みモーダルと素材追加メニューはSafariの表示領域に合わせ、入力・追加・閉じる操作を画面内に収める。
