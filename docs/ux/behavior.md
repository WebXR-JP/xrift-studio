...
- Playは追加のScript承認ダイアログを挟まず、保存済みのsourceを読み込み、変換に成功してから開始する（MI-70）。UIとMCPで同じ手順を使い、Scriptの更新後も再承認は求めない。準備中はEditを保ち、失敗した場合はConsoleから原因と対象fileを確認できる。Scriptの来歴とsource hashは診断情報として保持する。
