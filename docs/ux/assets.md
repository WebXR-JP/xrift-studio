...
- 歯車から開く設定パネルに「取り込むテクスチャのサイズと圧縮」を置く。Importメニューでは設定しない。未設定時は最大1024px・KTX2圧縮とし、Scene Viewへ配置する前に変換を完了する。保存済みの設定とMCPで明示したTexture設定は優先する。単体・モデル内蔵の対応画像に同じ上限で編集用画像を生成し、元ファイルを保持する。選択した既定値はEditor Stateとしてブラウザに残る。モデル再インポートにも適用し、個別に保護した設定を優先する。
...
噴水の詳細には、水面・水筋が静的モデルで、中央の飛沫だけが動くことを配置前に示します。既存ワールドへ取り込み済みのAssetは自動上書きしません。ブラウザの実描画で読み込み待ち・失敗・ジオメトリ解放を確認しました。Tauri実機とメモリ使用量の測定は未実施です。

### F-24 頂点カラーとOpacityマップ（2026-09-08）

参照: MI-03、MI-09。Diffuse（Base Color）の「頂点カラーを使用」で、モデルに保存された色を基本色へ乗算する。色を持たないモデルの表示は変わらない。OpacityマップはR・G・B・Aのいずれかを選び、AlphaとBase Color TextureのAへ乗算する。初期値はA。マップを初めて指定した際、OpaqueならBlendへ切り替え、同じ変更としてUndoできる。片面・両面は既存のDouble sidedで選ぶ。

操作前は現在の選択を表示する。読み込み中・失敗は既存Textureスロットの表示を使い、成功時はプレビューとSceneへ反映する。読み込みに失敗した場合は同じスロットから選び直せる。変更後もMaterialを選択したまま編集を続けられ、Undo・Redo・保存・再読み込みで設定を保持する。Unity独自Shaderを変換する操作ではなく、Studioの標準Materialの設定として扱う。MCPは既存のMaterial更新操作へ同じプロパティを渡す。
