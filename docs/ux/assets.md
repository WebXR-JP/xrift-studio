...
## F-40 公開画像の保持と取り込み時の最大解像度の状態設計

参照: MI-03, MI-05, MI-09, MI-15, MI-16, MI-25, MI-67, MI-100
...
- 公開は現在の画像ファイルをコピーするだけで、画像変換Workerを起動しない。公開直前に画像が変わる処理を追加しない。
- 取り込み時やInspectorから明示的に変換するときは、必要な画像を順番に処理する。KTX2圧縮は専用Workerで1枚ずつ実行し、画面スレッドを占有しない。完了・失敗時にWorkerを終了して圧縮用メモリを解放する。120秒で完了しない場合も終了し、最大解像度を下げるかWEBPで再試行する案内を出す。変換に失敗した場合は新しいManifestを採用せず元の参照を保持する。glTFのsampler由来の設定は取り込みの既定より優先する。HDRI・SVG・元画像を保持していないKTX2は変換しない。

### 成功時

- 公開されたテクスチャのバイト列・形式・解像度は現在の元データと一致する。適用済みのKTX2はKTX2として、未変換のPNGはPNGとして読み込む。
- 公開によって制作データ・元画像・読み込み設定を変更しない。明示的な加工で保持した元画像も残す。
- 容量とVRAMは、現在使用中の画像の形式・解像度から見積もる。未適用の設定を適用済みとして扱わない。

### 失敗時

- 公開用コピーの読み込み・保存に失敗した場合は対象を示して公開を止める。制作データは変更しない。
- 明示的な画像変換の失敗は、その操作の画面に理由を残す。現在の画像を維持し、同じ設定から再試行できる。

### 戻り先

- 公開画面を閉じると同じ編集画面へ戻る。画像を変えたい場合はInspectorで適用し、表示を確認してから公開画面を開き直す。

完了条件: 公開だけではテクスチャのバイト列・形式・解像度を変えない。明示的な加工と取り込み時の変換は、シーンとPlayで結果を確認できる。元ファイルを保持する。
...
- 「外部から追加 > ギミック」に操作付きセットを配布する。家具や装飾は「3Dセット」、glTFの表現見本は「マテリアル表現 > glTFマテリアル」、発光は「マテリアル表現 > Emissive」に分ける。分類を切り替えると検索・選択・結果表示を初期化し、対象の候補だけを表示する（MI-04）。配置前の詳細、追加中の無効化、成功後の選択、失敗時の再試行は共通の導線を使う。
...
### F-43 glTFマテリアル・ギミック各50種類への拡充（2026-09-08）

参照: MI-03、MI-04、MI-05、MI-09、MI-11。従来の追加処理、角丸、白・グレーの配色、主操作色を保ち、読みにくかった文字とプレビューを大きくした。

操作前は総数と該当件数、用途別の分類、英語名と日本語の説明を表示する。Emissiveの専用ボタンと「テクスチャ付き」で絞り込める。選択中の項目が絞り込みから外れたら、表示中の候補へ切り替える。見本の比較条件は3D内のラベルに加えて詳細ペインにも表示する。回転と拡大は見え方の確認用で、ギミックの操作は追加後のPlayで行う。
