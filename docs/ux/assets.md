...
- Importメニューに「取り込むTextureの最大解像度」を置く。未設定時は最大2048px・KTX2圧縮とし、Scene Viewへ配置する前に変換を完了する。保存済みの設定とMCPで明示したTexture設定は優先する。単体・モデル内蔵の対応画像に同じ上限で編集用画像を生成し、元ファイルを保持する。選択した既定値はEditor Stateとしてブラウザに残る。モデル再インポートにも適用し、個別に保護した設定を優先する。
