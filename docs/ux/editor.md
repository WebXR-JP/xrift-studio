...
- 操作前: 11カテゴリ・名前・説明・タグで絞り込み、選択が検索結果から外れたら先頭へ移す。0件は空状態を表示する。一覧は本体と同じThree.js Material生成処理で描いた静止画、詳細だけはR3Fで描画する。
- 操作中（MI-03）: 軽量・標準・高精細とUniformを詳細に反映する。追加中は検索・カテゴリ・カード選択・品質・Uniform・再追加を無効にする。
- 成功（MI-05）: 選んだ品質のdefinesをMaterialへ保存し、Sceneへの適用結果を残す。既存Materialは同じプリセットを追加し直した場合だけ更新する。
- 失敗・戻り先（MI-09）: 選択と設定を保持して再試行できる。詳細はドラッグ・矢印キー・再生停止・視点リセットに対応し、非表示中は連続描画を止める。
- MCP: `list_material_presets`で品質別variantsを取得し、`create_material_from_preset`と`update_custom_shader`で同じ品質を設定する。既存の編集操作で扱えるため専用toolは増やさない。プレビュー視点と停止はストアだけの状態で、Sceneへの保存やviewer overrideの対象ではない。
