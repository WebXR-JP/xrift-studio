...
- Graph 間はイベントで結合する。`event/send` で名前付きイベントを送り、別 Graph の `event/receive` が受ける。イベント名は Asset 内で一意とし、Editor は候補を列挙する
...
| Phase | 内容 | 完了条件 |
| --- | --- | --- |
| A | 実行エンジンの一本化 | 既存 8 operation が新エンジンで同じ結果を出し、既存 fixture が通る。Interactivity Asset の `event/onStart` が Play で走る |
| B | Operation 第一段階と順序制約の撤廃 | 待機、順次、繰り返し、合流、変数、算術が Play と公開先で同じ順序・同じ時刻に走る |
| C | Action 対象の拡張 | Transform、Animation、Material、Particle、Scene への書き込みが Play と公開先で一致する |
| D | エディタ操作の修復と拡張 | 線の削除・繋ぎ替え・ノード削除・Undo / Redo・コピー / ペーストが document と一致して動く |
| E | Graph 合成 | Graph の追加・複製・削除と、`event/send` / `event/receive` による結合が動く |
| F | タイムラインビューとデバッグ | Cue 一覧、スクラブ、実行トレース、診断からのノード移動が動く |
