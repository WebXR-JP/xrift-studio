...
- `instructions` の先頭に制作工程を置いた。ブリーフ (概念、ムード、地面の種類、主役、入れない
  もの) を先に決める。土台 → 主役 → 空と光 → 小物 → 検証の順に作る。Terrain は屋外の風景の
  選択肢の 1 つで既定の床ではない。草は背景で、撮ってから直す。動き・繰り返し・生成的な構造は
  Script で作る。完了報告の前にスポーン位置と iso の `capture_scene_view` を撮る。
- `create_primitive` を「ブロックアウトの道具」として説明し直し、床・壁・舞台・浮遊構造の
  材料であること、灰色の既定 Material のまま出荷しないことを書いた。
- `list_terrain_presets`、`create_terrain_from_preset`、`create_terrain` に「いつ使わないか」と、
  `grassPresetId: null` で草無しにできることを書いた。
- `apply_terrain_grass_preset`、`list_terrain_grass_types`、`update_terrain_grass_layer` に
  「草は背景」「調整はフレームに写った欠陥から始める」を書いた。回数の上限は書かない。
...
- `.agents/skills/xrift-world-direction/` を新設した。ブリーフの書式、工程の順番、見てから直す
  作法、完了の確かめ方、素材の調達順、Script の使いどころを持つ。テーマ、床の種類、素材の
  候補一覧、数値の上限、コードの雛形は持たない。案は 1 つに絞り、過去のワールドは参照しない。
...
- `AGENT.md` に、ワールドを作る作業はブリーフから始めること、tool の description には
  「いつ使わないか」と「次に何をするか」を書くこと、ハーネスに数値の上限とコードの雛形を
  入れないことを足した。
...
| 入れないもの | 理由 |
| --- | --- |
| 数値の上限 (回数、ライト数、三角形数) | 適切な値はモデルの性能と特性で変わる。上限はモデルがブリーフで自分の予算として決め、ハーネスは「決めたか」「守ったか」だけを見る |
...
| 複数案の提示と選択の工程 | 案は 1 つに絞って作り、ブリーフを最初の報告に入れて途中で直せるようにする |

## 運用の決まり

- tool を足す・直すときは、「何をするか」に加えて「いつ使わないか」「使ったあと何をするか」を
...
- 空 preset に合わせた Light preset (`WORLD_CONTENT_PLAN.md` の A-2)。空だけ変えて光が
  残る事故を tool 側で止める。
- ブリーフを project metadata として保存し、`get_editor_context` で返す。今はスキルの
