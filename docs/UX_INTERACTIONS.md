...
- 「自動」のまま公開したTextは、公開時に同梱の書体へ置き換えて出力する。自動の書体は文字種ごとに配布元から代替フォントを取得する仕組みで、公開したWorldはそこへ到達できないため、そのまま出すと文字が表示されない。置き換えた書体のファイルはWorldへ同梱し、Runtime JSON出力ではmanifestがその置き場所を示す。
