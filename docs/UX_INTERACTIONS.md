...
| MI-120 | Inspector、シーン設定、Material / Particle / Interactivity、`xrift.json` の数値を変える | 数値欄はブラウザ既定の上下スピナーを出さず、待機中は左右のリサイズカーソルで引けることを示す。入力欄を左右へ引くと値が動き、ポインタの近くに開始値と現在値を並べた読み取りだけの表示を出す。Shift は 1/10、Ctrl / Alt は 10 倍で動かす。動かさずに離した時は通常のクリックとして全選択し、そのまま数値を打てる。打った値はキーを押すごとに反映し、確定のために別の場所をクリックさせない。 | ドラッグ一回を一つの Command Transaction とし、pointer up で一件の履歴として確定する。Escape、pointer cancel では引き始める前の値へ戻し、履歴を追加しない。空欄にできる項目は解除、それ以外は開始値を保つ。Undo を持たない設定 (スナップ幅) は共通のトランザクションへ入れず、履歴も増やさない。
...
- Inspector とシーン設定の数値欄は上下スピナーを出さず、欄そのものを左右へ引いて値を動かす。動かさずに離した時はクリックとして数値入力へ入り、打った値はキーごとに反映する。ドラッグ一回は一件の履歴として確定し、Escape では引き始める前へ戻す。
