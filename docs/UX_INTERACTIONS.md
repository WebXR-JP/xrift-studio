...
| MI-120 | Scene設定のポストエフェクトでレイヤーの順番を入れ替える、または各レイヤーをまとめて有効・無効にする | ポストエフェクトを、適用順の番号付きレイヤー一覧として並べる。各行はレイヤー名、有効チェック、上下の並べ替え、そのレイヤー自身の値をまとめて持つ。AOはsceneを描き直すpassなので常に1番目に固定し、動かせない理由を行内に書く。Bloomと色味は上下ボタンで入れ替えられ、番号が即座に付け替わる。一覧の上に「すべて有効」「すべて無効」を置き、全体スイッチとは別に各レイヤーだけをまとめて切り替える。全体が無効のときは一覧全体を無効表示にする。 | 同じ値でも掛ける順番で仕上がりが変わるのに、以前は有効・無効しか選べず、順番はコード側の固定だった。順番をdocumentへ保存し、Scene View・Play・公開Worldが同じ順で合成する。順番の変更は一件のScene更新として保存し、MCPの`update_scene_settings`の`postprocessing.order`からも同じ配列を書ける。ビューアーごとの実行時上書きは有効・無効だけで、順番は作者の決めた見た目なので端末ごとには変えない。 |
