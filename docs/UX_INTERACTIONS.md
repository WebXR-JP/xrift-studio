...
| MI-95 | ノードエディターのウィンドウが、画面の幅によって形を変える | 左右は Hierarchy と Inspector の実際の幅に合わせて止まる。上は Scene View のツールバーを空ける。ヘッダーは名前と保存・閉じるの 1 行で高さが変わらない。道具は 2 段目の 1 行に置き、入りきらないときは横スクロールし、右端に薄いグラデーションを出して続きがあることを示す。フッターは 1 行のまま、幅が足りなくなった順に操作ヒント・色の凡例を引っ込める。 | 保存と閉じるは、どの幅でも必ず見える位置に置く。窓の右端を画面の端から固定距離で取ると、Inspector の下へ潜って保存が押せなくなる。ヘッダーを折り返すと、狭いほどキャンバスが減って、見せたいグラフが最初に消える。 |
