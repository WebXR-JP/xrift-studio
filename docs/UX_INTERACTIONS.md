...
OpenXRパネルは未確認・診断中・拡張不足・取得中・素材保存中・完了・失敗を区別する。診断が成功して必要な拡張を確認するまで取得ボタンを無効にする。拡張の存在は実機取得成功を意味しない。

主操作は「保存済みの部屋を取り込む」。取得中だけ取消を表示し、診断と取得・保存の同時実行を防ぐ。パネルを閉じても進行状態を保持する。成功時は配置Entityを選択し、寸法・向き・床位置の確認へつなぐ。失敗理由を残し、再診断・再試行できるようにする。取得中のScene変更・取消・画面終了では古い結果を適用しない。
