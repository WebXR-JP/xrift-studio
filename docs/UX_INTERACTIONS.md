...
| MI-74 | プロジェクト一覧の「ヘルプと報告」を開き、GitHub IssueまたはヘルプセンターGPTへ相談する、環境情報をコピーする、または現在の画面を保存する | 初回のプロジェクト一覧表示後に相談準備モーダルを表示し、GitHub / ChatGPTの主操作を並べる。環境情報はアプリ・OS・Node.js・@xrift/cli・CPU architecture・プロジェクト件数だけを表示し、アクセストークン、Cookie、パスワード、絶対パスは含めない。ChatGPT / GitHubを開く前に同じ情報をクリップボードへコピーする。スクリーンショット保存中は二重操作と閉じる操作を無効化する。 | 成功時は外部URLまたは保存先へ到達し、コピー成功時は貼り付け先を案内する。キャンセルはモーダルを閉じ、後からヘルプアイコンで再開できる。保存失敗時は原因を表示して同じ操作から再試行できる。 |
