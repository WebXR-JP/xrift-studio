...
アプリの使い方は[利用者向けガイド](./wiki/index.md)、開発環境の準備は[開発ガイド](../DEVELOPMENT.md)を参照してください。このページでは、実装を調べるための文書を案内します。
...
利用者向けの手順は`docs/wiki/`、実装上の条件は分野別の仕様書に置きます。READMEには最初の操作と案内だけを残し、機能ごとの説明を重複させません。過去の検証記録は、実施時点と未検証の範囲を残します。ライセンスや素材の権利表記は削除しません。

利用者向けページの名前を変えたときは、`src/lib/wiki-config.ts`と関連リンクを更新してください。コード・コマンド・保存形式の識別子は、画面の日本語に合わせて変更しません。
