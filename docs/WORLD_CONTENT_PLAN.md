...
| ID | 名前 | 内容 | なぜ要るか |
| --- | --- | --- | --- |
| A-1 | **Particle preset カタログ** | 焚き火の炎、煙、湯気、雪、雨、桜、蛍、火花、紙吹雪、魔法陣。「外部から追加」へ空・水と同じ並びで置く | Particle だけが白紙から。同じ導線に乗せるだけで初回の見栄えが変わる |
...
1. **A-1 Particle preset カタログ** から始める。空と水で「カタログから選べば良くなる」体験ができているのに、Particle だけが取り残されている。ここが一番安く、一番効く。
2. 次に **B-1 焚き火**。A-1 があれば値を決めるだけで、「置くだけで様になる物」の最初の一つになる。
