...
| B-2 | **街灯**（完了） | 台座 + 支柱 + 笠 + 光る球 + 歩道に届く距離の Point Light |
| B-3 | **看板** | 板 Mesh + Text + 任意の BillboardY。Text の runtime は troika なので、カードの描画経路を用意してから着手する |
| B-4 | **松明**（完了） | 細い柄 + 小さな炎 + 火の粉 + 手元を照らす Point Light |
| B-5 | **水辺セット** | Water Material の面 + 岸の岩 + 波の環境音 |
| B-6 | **入口ゲート** | Portal + 装飾アーチ + 手前の Spawn Point |
| B-7 | **ベンチ**（完了） | 座面 + 脚 2 + 背もたれ。座る機能は C-4 が入ってから |
| B-8 | **提灯**（完了） | 横木 + 光る球 3 + 中央に 1 灯 |
| B-9 | **蛍の茂み**（完了） | 茂み 3 + 蛍 Particle + 弱い緑の Point Light |
| B-10 | **噴水**（完了） | 石の縁 12 + 中央の台 + 飛沫 Particle |
...
2. ~~**B-1 焚き火**~~ 完了。「セット」provider を作り、焚き火・松明・提灯・街灯・蛍の茂み・噴水・ベンチの 7 件を置いた。以降も `SceneRecipe` にデータを足すだけで並ぶ。
3. 次は **C-1 Audio Zone** か **C-2 Weather**。どちらも Wind の契約をなぞれるので設計を新しく起こさずに済む。セットを増やす場合は、Text を含む B-3 看板の前に、カードで Text を描く経路を決める必要がある。
