...
初めてのワールド制作で、最初の一時間に必要になるものを並べた計画である。設計は [ビジュアルエディター設計](./VISUAL_EDITOR_ARCHITECTURE.md) を正とする。現在の対応範囲は [対応範囲と段階](./VISUAL_EDITOR_ROADMAP.md) を正とする。この文書は、そこへ何を足すかだけを扱う。
