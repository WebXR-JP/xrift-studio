...
| Classic source graphからVisualへの取り込み | 開発版あり | local folderまたはHTTPS／git SSH Repositoryを検査し、World／Item entryから`src`のrelative importを再帰解決する。静的R3F、Rapier、XRift Component、Model／Texture／Audio、Skybox、ShaderMaterialをコード実行なしでlossy変換し、確定前reviewと確定後Playへ接続する。`group`、wrapper、local Componentの親子境界を維持し、動的分岐／collectionは診断対象にする。 |
...
- MP3／WAV Audio AssetとAudio Sourceの参照、配置、compileを追加した。
- Classic folder／Repositoryの確定前review、関連Asset、Skybox、環境音、Custom Shader Material、named OBJ Colliderの復元と確定後Playを追加した。
...
repository内の開発版はRuntime JSON、Three.js／R3F adapter、dry-run、未改変exportのupdateに加え、Visual Editorから既存Classic projectを選ぶ導線まで接続済みである。npm公開とRuntime Componentの対応範囲は[Visual Project Classic Export CLI](./VISUAL_PROJECT_MIGRATION_CLI.md)にまとめる。
