...
| Classic source graphからVisualへの取り込み | 開発版あり | 検査済みWorld／Item entryから`src`のrelative importを再帰解決し、静的R3F、Rapier、XRift Componentを実行せずlossy変換する。`group`、wrapper、local Componentの親子境界を維持する。Hook、動的分岐／collection、Asset依存、runtime stateは診断対象で、完全round-tripではない。 |
