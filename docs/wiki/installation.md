...
ビジュアルエディターだけを始める場合は、**セットアップせずに作り始める**からワールドかアイテムを選べます。編集と保存はそのまま行えます。公開時にはセットアップが必要です。

## 3. セットアップを開始する

セットアップ画面で **セットアップを開始** を押します。

XRift Studio は、システムにインストール済みの Node.js、npm、`@xrift/cli` を**原則として使用しません**。アプリ専用フォルダーに、次のものを隔離して準備します。
...
セットアップが完了すると、**プロジェクト一覧**が表示されます。ここから新しいプロジェクトを作成できます。

次のステップは [プロジェクトを作る・開く](./projects.md) を参照してください。
...
[トラブルシューティング](./troubleshooting.md) と [データの保存場所とリセット](./data-and-reset.md) を参照してください。
