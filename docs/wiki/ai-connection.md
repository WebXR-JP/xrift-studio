...
AI は指示が少ないと、草原の地形と木から作り始めがちです。Studio の MCP には「先に 5 行の
設計メモを書いてから作る」手順が組み込まれていますが、頼み方でも仕上がりが変わります。

- 場所ではなく、そこで何をするかを伝える。「森を作って」より「夜、友人 4 人で焚き火を囲んで話す広場。木は少なめ」。
- 入れたくないものを先に言う。「地形と草は使わない」「屋内だけ」。
- 完了の条件を言う。「スポーン位置からのスクリーンショットを撮って見せて」。
- AI が最初に出す設計メモ (誰が来て何をする場所か、雰囲気、床の種類、主役、入れないもの) を読んで、違えばその場で直す。
