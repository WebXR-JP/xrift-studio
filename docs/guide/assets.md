...
車は **外部から追加 → ギミック → カスタム車** を選び、**カスタム車をシーンへ追加**を押します。**XRift公式コンポーネント → Vehicle** からも、同じ設定済みの車を追加できます。椅子単体は **XRift公式コンポーネント → Seat** から追加できます。

ギミック・3Dセット・マテリアルのカード一覧は基本4列です。一覧の幅が狭い場合は3列・2列に切り替わります。

カスタム車は屋根のない車体・運転席・同乗席・4本のタイヤ・排気煙が、設定済みのHierarchyとして配置されます。車体や座席は編集画面でも3Dで確認できます。Hierarchyで子Entityを選び、Inspectorで位置やマテリアル、Scriptの設定を調整してください。座席の高さは、座席EntityのTransformで変更します。

モデルはプロジェクトのAssetsにGLBとして追加されます。色は通常のglTFマテリアルのBase Colorを使い、座席は1マテリアルです。色や質感はMesh Rendererから変更できます。
