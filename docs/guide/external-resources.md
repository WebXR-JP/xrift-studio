...
素材を探して取り込む入口は、上部の**素材を追加 → 外部から追加**です。**Assets → 外部から追加**からも同じカタログを開けます。使いたい見た目や機能から選びます。
...
ブラウザ版では**音蔵**の効果音・環境音を直接追加できます。音蔵を選んで音源を検索し、詳細の**プロジェクトに追加**を押すと、OGG音源がAssetsへ保存されます。追加した音源はAudio Sourceに設定できます。Poly HavenとambientCGからの直接追加はデスクトップ版で利用できます。
...
**ギミック**はワールドとアイテムのどちらにも追加できます。音の出るボタン、灯りのスイッチ、自動で閉まる扉などの見本があります。まず詳細の説明を読んで配置し、**Play**で動きを確かめます。プレイヤーやルームが必要な動作は、アイテムの配置先ワールドでも確認してください。
