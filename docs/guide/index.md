...
iPadで始める方は、[iPadで作り、パソコンへ引き継ぐ](./ipad.md)をご覧ください。ブラウザで編集し、ZIPをMacまたはWindowsへ渡して公開する流れです。
...
基本の制作手順はデスクトップ版を対象にしています。iPadの画面・タッチ操作・保存方法は[iPadの案内](./ipad.md)で補います。スマートフォンは制作画面の対象に含めていません。ブラウザへの保存、ZIPへの書き出し、XRiftへの公開は別の操作です。
