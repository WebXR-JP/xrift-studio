...
追加先の `projectKind` を切り替えない。例えばワールドの家具をアイテムにする場合は、アイテムプロジェクトを作成して、そのHierarchyへ家具を追加する。以降の編集・公開は既存のアイテム向け経路を使う。
...
Hierarchyの操作メニューと上部の「素材を追加」から新しい確認dialogへ入る。dialogはnative `<dialog>` のmodal/focus管理を使い、処理中は変更・終了・二重実行を防ぐ。Entityの選択一覧は描画数を制限し、検索に対応する。Import完了までは確認画面を閉じず、失敗理由を残す。
