# 水面シェーダー v2 — 海・水面カタログの拡充

## 変更の概要

既存の水面シェーダーを3種類から21種類、8カテゴリへ拡充しました。旧3種類のIDを維持し、新規18種類を追加しています。GLSLは共通コアと演出用の定義に分かれ、16通りの演出構成があります。同じ波に色だけを変えた21個ではありません。

素材の「外部から追加 → 水面シェーダー」と、既存MCPの`list_material_presets` / `install_material_preset`から使います。インストール後は従来どおり、GLSLとUniformを保持した独立したマテリアルになります。新しいネットワーク取得先や依存パッケージは追加していません。

## 使い方

1. この変更を取り込んだXRift Studioを、元のプロジェクトで使用している依存関係を導入して起動します。
2. 素材 → 外部から追加 → 水面シェーダーで、検索またはカテゴリから選びます。
3. 詳細プレビューを見ながら波・さざ波、反射・透明感、泡、演出、色を調整し、「マテリアルへ追加」を押します。
4. 作成した水平の水面メッシュにマテリアルを割り当てます。追加後の調整は設定の調整できる値を使います。
5. 動かない場合はシーンの風を確認します。詳細パネルの「試し風」はプレビューだけに効き、シーンは変更しません。
...
| 名前 | ID | 主な表現 |
| --- | --- | --- |
| 穏やかな湖 | calm-lake | 細かなさざ波・静かな反射 |
| 波のある海 | ocean-waves | 交差するうねり・泡の網目・逆光の波頭 |
| トゥーン調の海 | stylized-toon | 段階的な陰影・波の輪郭線 |
| 外洋の大きなうねり | deep-swell | 長いうねり・4層の波・深い青 |
| 風が渡る海 | trade-wind-sea | 方向性のある波・風下への泡筋 |
| 南国の浅い入り江 | tropical-lagoon | 青緑の浅瀬・疑似コースティクス・小さな波 |
| サンゴ礁の浅瀬 | coral-shallows | 寄せ波・明るい浅瀬・光の網目 |
| 海岸の打ち寄せる波 | coastal-surf | 往復する波打ち際・砕ける波・浅瀬の色変化 |
| 磯に寄せる波 | rocky-coast | 交差波・大きな寄せ波・細長い泡 |
| 嵐の海 | storm-sea | 荒いうねり・崩れる白波・風の泡筋 |
| 雨の港 | rainy-harbor | 雨の円形波紋・静かなうねり・曇天の反射 |
| 夕日に染まる海 | golden-tide | 低い光のきらめき・暖色の水平線・長いうねり |
| 月明かりの海 | moonlit-ocean | 月明かりの反射・暗い波の谷・控えめな白波 |
| 夜光虫の入り江 | bioluminescent-bay | 発光する寄せ波・光る微粒子・暗い水面 |
| アニメ調の夏の海 | anime-ocean | 鮮明な色面・白い波線・浅瀬の光 |
| 水彩画の海 | watercolor-sea | 絵の具のむら・かすれた泡・柔らかな反射 |
| 墨絵の波 | ink-waves | 墨の等高線・斜線の陰影・紙色の泡 |
| オーロラ色の海 | aurora-tide | 角度で変わる色・発光する波頭・光る微粒子 |
| ホログラムの海 | holographic-sea | 波で歪む格子・流れる光・発光する輪郭 |
| 虹色の海 | iridescent-sea | 角度依存の虹色・なめらかな反射・交差する波 |
| 氷河の海 | glacial-sea | 流氷風の模様・氷の割れ目・穏やかな水面 |
...
風の速度、`uWaveSpeed`のどちらかが0なら、水面の波・泡・雨の波紋・夜光の動きが止まります。これは既存の共有風契約を維持するための動作です。

## 表現上の制約

反射は指定した天頂色・水平線色と水専用の雲模様の近似です。シーン内の建物やアバターを鏡のように映す機能ではなく、空の背景の画像・雲とも自動同期しません。夕景・月明かりのサンプル照明は、そのままシーンへ追加されません。実際の主光源と環境光はシーンの設定から供給されます。
...
既存マテリアルのGLSLは保存データに埋め込まれているため、ソースを更新するだけでは自動変更されません。同じプリセットをカタログから追加し直すと、従来のIDのマテリアルが現在のパラメータで更新されます。以前の見た目を保持したい場合は先にプロジェクトを保存し、別名バックアップを残してください。

この成果物は共有された`xrift-studio-main(1).zip`を基準にした海用の改修です。別作業の空の背景や3Dモデル改修を取り込んだものではありません。それらと組み合わせる場合は、プロジェクト丸ごとの上書きではなく差分パッチを確認して適用してください。

## 取り込み時の検証（2026-09-07）

最新mainへ海関連の差分だけを統合しました。`pnpm typecheck`、`pnpm e2e:typecheck`、カタログ検査（21種類・532パラメータ）、`e2e/water-shaders.spec.ts`の2テストを通過しています。Three.jsの共通プレビュー描画で全21種類を品質3段階の計63通り確認し、高精細では頂点変位も有効にしました。既存のマテリアル追加・生成ワールドへのGLSLと風の出力に関するfixtureも通過しています。
