...
- ユーザーがMCP tool名を明示した依頼では、そのtoolが現在の公開tool一覧に存在しない場合、似たtoolやPrimitiveで代替実行しない。XRift Studioの「AI接続」からMCPを更新し、AIクライアントを再起動または再読み込みするよう案内する。特に decide_fast_authoring 指定時はこのルールを守る。
