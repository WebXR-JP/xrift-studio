...
const {Matrix4: Matrix4$8,Plane: Plane$1,Sphere: Sphere$3,Vector2: Vector2$4,Vector3: Vector3$7} = await importShared('three');
...
function makeClippedCast(component, fn, root, parent, orderInfoSignal) {
  return (raycaster, intersects) => {
    ...
    outer: for (let i = intersects.length - 1; i >= oldLength; i--) {
      ...
      intersection.distance -= orderInfo.majorIndex * 0.01 + orderInfo.minorIndex * 1e-4 + //1-100
      orderInfo.elementType * 1e-5 + //1-10
      orderInfo.patchIndex * 1e-7;
      if (clippingPlanes == null) {
        continue;
      }
      for (let ii = 0; ii < 4; ii++) {
        planeHelper.copy(clippingPlanes[ii]).applyMatrix4(worldToGlobalMatrixHelper$1);
...
const {Color: Color$1,FrontSide,MeshDepthMaterial: MeshDepthMaterial$1,MeshDistanceMaterial: MeshDistanceMaterial$1,RGBADepthPacking} = await importShared('three');
    ...
    return {
        ...
        computedIsVisibile: (properties, borderInset, size, isVisible) => {
            return g(() => {
                ...
                const borderVisible = borderInset.value.some((s) => s > 0) && colorArrayHelper$1[3] > 0;
...
const colorHelper$1 = new Color$1();
...
const {InstancedBufferAttribute: InstancedBufferAttribute$1,DynamicDrawUsage: DynamicDrawUsage$1,MeshPhongMaterial,MeshPhysicalMaterial} = await importShared('three');
...
const positionHelper$2 = new Vector3$6();
class ClippingRect {
  planes;
  facePlane;
  originalCenter;
  ...
  min({ planes }) {
    for (let i = 0; i < 4; i++) {
      ...
      positionHelper$2.copy(p1.normal).multiplyScalar(c1).addScaledVector(p2.normal, c2);
      helperPlane2.constant = -positionHelper$2.dot(helperPlane2.normal);
    }
    let indexOffset = 0;
...
const setter = { positionType: (root, node, input) => {
...
let PropertiesImplementation$1 = class PropertiesImplementation {
    ...
    peekValue(key) {
        ...
        const [result] = o(() => selectLayerValue(0, layerIndices, this.propertiesLayers, key, defaultValue));
        return result;
    }
...
function computedCustomLayouting(properties, fontSignal, propertiesRef) {
    return g(() => {
...
const {Matrix4: Matrix4$5} = await importShared('three');
const helperMatrix1 = new Matrix4$5();
const helperMatrix2 = new Matrix4$5();
...
function createInstancedText(text, parentClippingRect, selectionRange, selectionTransformations, caretTransformation, instancedTextRef) {
...
class InstancedText {
  ...
  constructor(group, properties, layoutSignal, matrix, isVisible, parentClippingRect, selectionRange, selectionTransformations, caretTransformation) {
    ...
    this.unsubscribeInitialList = [
      ...
      j(() => this.updateSelectionBoxes(this.lastLayout, selectionRange?.value, properties.peek().verticalAlign, properties.peek().textAlign))
    ];
  }
  ...
  updateSelectionBoxes(layout, range, verticalAlign, textAlign) {
    ...
    if (range == null || layout == null || layout.lines.length === 0) {
      this.caretTransformation.value = void 0;
    ...
    if (endCharIndexExcl <= startCharIndexIncl) {
      ...
      const y = -(getYOffset(layout, verticalAlign) - layout.availableHeight / 2 + lineIndex * getOffsetToNextLine(layout.lineHeight) + getGlyphOffsetY(layout.fontSize, layout.lineHeight));
    ...
    const newSelectionTransformations = [
      this.computeSelectionTransformation(start.lineIndex, start.x, void 0, layout, verticalAlign, whitespaceWidth)
    ];
    for (let i = start.lineIndex + 1; i < end.lineIndex; i++) {
      newSelectionTransformations.push(this.computeSelectionTransformation(i, void 0, void 0, layout, verticalAlign, whitespaceWidth));
    }
    newSelectionTransformations.push(this.computeSelectionTransformation(end.lineIndex, void 0, end.x, layout, verticalAlign, whitespaceWidth));
    ...
    this.unsubscribeShowList.push(j(() => {
      ...
      for (let lineIndex = 0; lineIndex < linesLength; lineIndex++) {
        ...
        for (let charIndex = firstNonWhitespaceCharIndex; charIndex < firstNonWhitespaceCharIndex + charLength; charIndex++) {
          ...
          x += getKerningOffset(font, fontSize, prevGlyphId, glyphInfo);
          glyph.updateGlyphAndTransformation(glyphInfo, x + getGlyphOffsetX(glyphInfo, fontSize), -(y + getGlyphOffsetY(fontSize, lineHeight, glyphInfo)), fontSize, pixelSize);
          glyph.show();
          prevGlyphId = glyphInfo.id;
...
function buildRootMatrix(properties, size) {
    ...
    const [width, height] = size.value;
    const pixelSize = properties.value.pixelSize;
    return new Matrix4$4().makeTranslation(alignmentXMap[properties.value.anchorX] * width * pixelSize, alignmentYMap[properties.value.anchorY] * height * pixelSize, 0);
}
...
const {Matrix4: Matrix4$3,Mesh: Mesh$2,Sphere} = await importShared('three');
const IdentityMatrix$2 = new Matrix4$3();
const sphereHelper = new Sphere();
const worldToGlobalMatrixHelper = new Matrix4$3();
...
class Component extends Mesh$2 {
  ...
  updateWorldMatrix(updateParents, updateChildren) {
    ...
    computeWorldToGlobalMatrix(this.root.peek(), worldToGlobalMatrixHelper);
    this.matrixWorld.multiplyMatrices(worldToGlobalMatrixHelper, this.globalPanelMatrix.peek() ?? IdentityMatrix$2);
...
function computedBorderInset(properties, keys) {
  return g(() => keys.map((key) => properties.value[key] ?? 0));
}
...
const {Box2: Box2$1,Matrix4: Matrix4$2,Vector2: Vector2$3,Vector3: Vector3$4} = await importShared('three');
const distanceHelper = new Vector3$4();
const localPointHelper = new Vector3$4();
function computedGlobalScrollMatrix(properties, scrollPosition, globalMatrix) {
  return g(() => {
    ...
    const pixelSize = properties.value.pixelSize;
    return new Matrix4$2().makeTranslation(-scrollX * pixelSize, scrollY * pixelSize, 0).premultiply(global);
  });
}
...
function setupScrollHandlers(target, container, abortSignal) {
  ...
  abortableEffect(() => {
    ...
    target.value = {
      onPointerDown: (event) => {
        ...
        const scrollbarAxisIndex = ponterIsMouse ? getIntersectedScrollbarIndex(localPoint, container.properties.peek().scrollbarWidth, container.size.peek(), container.maxScrollPosition.peek(), container.borderInset.peek(), container.scrollPosition.peek()) : void 0;
...
function setupScrollbar(container, primaryIndex, parentClippingRect, orderInfo, groupDeps, borderSize) {
  const scrollbarTransformation = g(() => computeScrollbarTransformation(primaryIndex, container.properties.value.scrollbarWidth, container.size.value, container.maxScrollPosition.value, container.borderInset.value, container.scrollPosition.value));
...
class Image extends Component {
  ...
  constructor(inputProperties, initialClasses, inputConfig) {
    ...
    abortableEffect(() => {
      ...
      const cleanupSizeEffect = j(() => void (this.size.value != null && data.set(this.size.value, 14)));
      const cleanupBorderEffect = j(() => void (this.borderInset.value != null && data.set(this.borderInset.value, 0)));
      this.root.peek().requestRender?.();
      return () => {
        cleanupSizeEffect();
        cleanupBorderEffect();
...
const {Matrix4: Matrix4$1,Quaternion: Quaternion$1,Vector3: Vector3$3} = await importShared('three');

const textDefaults = { ...componentDefaults, ...additionalTextDefaults };
const IdentityMatrix$1 = new Matrix4$1();
const IdentityQuaternion$1 = new Quaternion$1();
const IdentityScale = new Vector3$3(1, 1, 1);
const positionHelper$1 = new Vector3$3();
class Text extends Component {
  ...
  constructor(inputProperties, initialClasses, inputConfig) {
    ...
    this.globalTextMatrix = g(() => {
      ...
      const pixelSize = this.properties.value.pixelSize;
      positionHelper$1.set((leftInset - rightInset) * 0.5 * pixelSize, (bottomInset - topInset) * 0.5 * pixelSize, 0);
      return new Matrix4$1().compose(positionHelper$1, IdentityQuaternion$1, IdentityScale).premultiply(this.globalMatrix.value ?? IdentityMatrix$1);
    });
    ...
    const customLayouting = createInstancedText(this, parentClippingRect, inputConfig?.selectionRange, inputConfig?.selectionTransformations, inputConfig?.caretTransformation, inputConfig?.instancedTextRef);
...
const cancelSet = /* @__PURE__ */ new Set();
...
class Input extends Text {
  ...
  constructor(inputProperties, initialClasses, inputConfig) {
    ...
    const selectionTransformations = y([]);
    const caretTransformation = y(void 0);
    const instancedTextRef = {};
    const selectionRange = y(void 0);
...
const segmenter = typeof Intl === "undefined" ? void 0 : new Intl.Segmenter(void 0, { granularity: "word" });
function setupSelectionHandlers(target, properties, text, component, instancedTextRef, focus, abortSignal) {
  abortableEffect(() => {
    ...
    target.value = {
      onPointerDown: (e) => {
        ...
        e.stopImmediatePropagation?.();
        if ("setPointerCapture" in e.object && typeof e.object.setPointerCapture === "function" && e.pointerId != null) {
      ...
      onPointerCancel: onPointerFinish,
      onPointerMove: (e) => {
        if (dragState == null || dragState?.pointerId != e.pointerId || e.uv == null || instancedTextRef.current == null) {
          return;
        }
        e.stopImmediatePropagation?.();
...
function createHtmlInputElement(selectionRange, onChange, multiline) {
...
class Video extends Image {
    ...
    constructor(inputProperties, initialClasses, inputConfig) {
        ...
        this.inputConfig = inputConfig;
        const srcIsElement = g(() => this.properties.value.src instanceof HTMLVideoElement);
...
export { inputDefaults as A, setupSelectionHandlers as B, Component as C, createHtmlInputElement as D, contentDefaults as E, Fullscreen as F, updateVideoElementSrc as G, componentDefaults as H, Image as I, PropertiesImplementation as P, Svg as S, Text as T, Video as V, getPreferredColorScheme as a, basedOnPreferredColorScheme as b, canvasInputProps as c, Container as d, Content as e, Custom as f, g, Input as h, isDarkMode as i, j, Textarea as k, l, TTFLoader as m, reversePainterSortStable as n, abortableEffect as o, searchFor as p, resetGlobalProperties as q, readReactive as r, setPreferredColorScheme as s, setGlobalProperties as t, imageDefaults as u, textDefaults as v, withOpacity as w, StyleSheet as x, ClassList as y, getStarProperties as z };
