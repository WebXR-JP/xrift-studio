...
description: XRift Studio の機能追加や IPC・CLI 連携を変更するときに使う。
---
...
1. UI の導線を変更する場合の設計: xrift-studio-ux スキルと docs/UX_PRINCIPLES.md に沿って「操作前・処理中・成功・失敗・戻り先」を決める。対応する動きを docs/UX_INTERACTIONS.md の MI-xx として記録する。
2. ブラウザで動く部分から実装: React の状態と表示を先に作る。この段階では Tauri API を呼ばない。
3. ネイティブが必要になったら IPC を分離: 下の「Tauri コマンドの追加手順」に従う。
4. 検証: xrift-studio-verify スキルから影響に合う Tier を選ぶ。UI を伴わない変更では画面設計・プレビューを省く。
