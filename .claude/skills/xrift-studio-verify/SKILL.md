...
description: XRift Studioのコード、画面、IPCを変更対象に合わせて検証・デバッグする。
---
...
- 目的、対象OS、所要時間の見込みを伝える。現在の依頼で許可済みなら再確認せず、未許可の場合に確認する。
...
TypeScript変更では実行する。文書・スキルだけの変更はリンク・frontmatter・コピーの同期を確認すればよい。Rustが参照する同梱ファイルを変更した場合はcargo testも対象にする。
失敗が依頼の差分に起因する場合は修正し、影響する検証を再実行する。独立した画面確認まで一律に止めない。各Tierは全件必須ではなく、対象の挙動を確認できるものを選ぶ。
XRはHMDなしの型チェックと実機でのセッション・描画・入力確認を区別する。
