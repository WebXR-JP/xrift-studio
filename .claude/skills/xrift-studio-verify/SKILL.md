...
必要な Bash コマンドと Tauri MCP ツールは `.claude/settings.json` で事前許可済みなので、確認を挟まず実行してよい。
...
`.claude/launch.json` の `web` サーバー（vite, port 1420）を preview_start で起動する。起動済みなら再利用される。vite の HMR が効くので、ファイル保存のたびに再起動は不要。
