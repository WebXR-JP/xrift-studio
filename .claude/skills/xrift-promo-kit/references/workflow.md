...
フィールドの詳細は [storyboard リファレンス](./storyboard.md)、演出の数値は [シーンと演出](./scenes.md) にある。
...
新しいシーン種別や演出が必要になったら、動画プロジェクト側に書かずキットへ足す。手順は [キットの拡張](./extending.md) にある。1 本の動画のためだけの実装をプロジェクト側に残さない。
