    ...
    >
      <div className="flex h-8 shrink-0 items-center gap-3 border-b border-slate-200 bg-slate-50 px-3 text-[11px] text-slate-600">
        <button type="button" aria-expanded={explorerOpen} aria-controls="script-explorer" onClick={() => setExplorerOpen((value) => !value)} className="rounded px-1 py-0.5 hover:bg-slate-200 focus-visible:outline-2">
          スクリプト一覧 {explorerOpen ? "を隠す" : "を表示"}
        </button>
        <span className="ml-auto">TypeScript{editor.playing ? " · 動作確認中" : ""}</span>
      </div>
      <div className="flex min-h-0 flex-1">
        {explorerOpen ? (
          <aside id="script-explorer" aria-label="スクリプト一覧" className="flex w-52 max-w-[35%] shrink-0 flex-col border-r border-slate-200 bg-slate-50">
            <div className="flex flex-wrap items-center gap-2 px-3 py-2 text-[11px]">
              ...
              <button type="button" onClick={onCreate} disabled={createDisabled || saving} title={createDisabled ? "プロジェクトを保存し、動作確認を停止すると作成できます" : "新規スクリプト"} className="ml-auto rounded px-1.5 py-1 text-brand-700 hover:bg-brand-50 disabled:opacity-40">新規スクリプト</button>
            </div>
            <div className="px-2 pb-2">
              <input type="search" aria-label="スクリプトを検索" placeholder="名前・パスで検索" value={query} onChange={(event) => setQuery(event.target.value)} className="w-full rounded border border-slate-300 bg-white px-2 py-1 text-xs outline-none focus:border-brand-500" />
            </div>
            <nav className="min-h-0 flex-1 overflow-y-auto" aria-label="スクリプトファイル">
              ...
              {visibleScripts.length === 0 ? <p className="px-3 py-4 text-xs text-slate-500">{scripts.length ? "一致するスクリプトはありません。検索語を変えてください。" : "スクリプトはありません。「新規スクリプト」から作成できます。"}</p> : null}
            </nav>
          </aside>
        ) : null}
