export const XRIFT_STUDIO_REPOSITORY_URL =
  "https://github.com/WebXR-JP/xrift-studio";

export const XRIFT_STUDIO_ISSUES_URL =
  `${XRIFT_STUDIO_REPOSITORY_URL}/issues`;

export const XRIFT_STUDIO_NEW_ISSUE_URL =
  `${XRIFT_STUDIO_ISSUES_URL}/new`;

// Publish the custom GPT, then replace this value with its shared URL.
export const XRIFT_STUDIO_BUG_REPORT_GPT_URL = "";
