...
import { createSpatialCapture, migrateSpatialCapture, normalizeSpatialPose, type SpatialSurface } from "./spatial-capture";
...
export function nativeRoomToCapture(room: NativeRoom) {
  ...
  const surfaces: SpatialSurface[] = room.surfaces.map(s => {
    ...
    const pose = normalizeSpatialPose({ position: s.position, rotation: s.rotation });
    ...
    if (s.boundaryXY && s.boundaryXY.length >= 3) {
      const rotation = new Quaternion(...pose.rotation).multiply(rotatePlane).toArray() as [number, number, number, number];
