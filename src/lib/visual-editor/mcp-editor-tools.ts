...
function updateSceneSettings(
  ...
  if (!sections.some((section) => argumentsValue[section] !== undefined)) {
    invalidArgument(
      "Scene settings",
      "shadowMapType、skybox、fog、ambient、camera、postprocessing、vegetation、physics、editorのいずれかを含むobject",
    );
  }
  const settings: SceneSettings = {
    shadowMapType: argumentsValue.shadowMapType === undefined
      ? currentSettings.shadowMapType
      : argumentsValue.shadowMapType === "basic" || argumentsValue.shadowMapType === "pcf" || argumentsValue.shadowMapType === "pcfSoft" || argumentsValue.shadowMapType === "vsm"
        ? argumentsValue.shadowMapType
        : invalidArgument("shadowMapType", "basic、pcf、pcfSoft、vsmのいずれか"),
    skybox:
