...
export function runRuntimePackageFixtureAssertions(): void {
  ...
  for (const react of ["^18.3.1", "^19.2.4", "19.3.0", "~19.2.4"]) {
    ...
    assert(manifest.dependencies["@xrift/world-components"] === "0.52.0", "old Components survived");
  ...
  const current = { dependencies: { "@xrift/world-components": "^0.52.0" } };
  recordCompilerPackageSpecs(current, specs);
  assert(current.dependencies["@xrift/world-components"] === "^0.52.0", "compatible author range changed");
