...
import { readGuide, guideContent, projectRoot, sourceRoot, escapeHtml as e } from "./core.mjs";
import { pageTemplate, contentsHtml } from "./template.mjs";
export async function buildGuide({ outDir = "preview-dist/guide", renderer } = {}) {
  ...
  if (!relative || relative.startsWith("..") || path.basename(out) !== "guide") throw new Error("Guide output must be a guide directory inside this project");
  const data = await readGuide(), content = guideContent(data);
  const render = renderer ?? (await import("./render.mjs")).renderMarkdown;
  // Render before replacing the generated directory.
  const pages = new Map();
  for (const page of data.manifest.pages) pages.set(`${page.slug}.html`, pageTemplate(page, data.manifest, render(data.sources[page.slug], page, data.manifest), content.headings[page.slug]));
  ...
  pages.set("404.html", special("404", "ページが見つかりません", "URLが変わったか、ページが存在しない可能性があります。", '<p>検索か目次から、目的の説明を探してください。</p><p><a href="./contents.html">目次から探す</a> · <a href="./index.html">ガイドのトップへ</a></p>'));
  ...
  await fs.writeFile(path.join(out,"search-index.json"),JSON.stringify(content.search));
  ...
  console.log(`Guide: ${data.manifest.pages.length} articles, ${pages.size} HTML pages, ${content.search.length} search sections → ${relative}`);
  return {out, pages:pages.size, articles:data.manifest.pages.length, sections:content.search.length};
}
